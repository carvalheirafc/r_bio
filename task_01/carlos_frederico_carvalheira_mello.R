...
# Unchar Count has only na values
df_6 <- select(df_6, -`Unchar Count`)
# Correlation Matrix Using Spearman Method
df_6_cor_matrix <- cor(df_6, method='spearman')
melted_df_6_cor_matrix <- melt(df_6_cor_matrix)
# Plot Using HeatMap
ggplot(data = melted_df_6_cor_matrix, aes(x=Var2, y=Var1, fill=value)) + 
  geom_tile() + 
  labs(x='', y='') +
  geom_text(aes(label=round(value, 1)), color='white', fontface=1, size=8) +
  ggtitle('Heatmap of Correlation between Quantitative Variables | Spearman') + 
